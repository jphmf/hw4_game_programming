...
void BugginOutDataLoader::loadWinScreen(Game *game)
{
	// WE'RE JUST GOING TO IGNORE THE GUI FILE FOR NOW.
	// FOR THE MOMENT WE ARE CALLING THIS HARD-CODED GUI LOADER
	WinScreen(game);
...
void BugginOutDataLoader::WinScreen(Game *game)
{
    GameGUI *gui = game->getGUI();
	GameGraphics *graphics = game->getGraphics();
	DirectXTextureManager *guiTextureManager = (DirectXTextureManager*)graphics->getGUITextureManager();
    ...
    unsigned int imageID = guiTextureManager->loadTexture(W_WIN_IMAGE_PATH);
    OverlayImage *imageToAdd = new OverlayImage();
    imageToAdd->x = 100;
	imageToAdd->y = 80;
	imageToAdd->z = 0;
	imageToAdd->alpha = 200;
    imageToAdd->width = 1176;
	imageToAdd->height = 882;
	...
	gui->addScreenGUI(GS_WIN_GAME, winGUI);
}
